...
import { faCopy, faKey, faSave, faLink, faEdit, faTrash, faEye, faEllipsisV } from '@fortawesome/free-solid-svg-icons';
...
const UrlCard = ({ urlData }) => {
    ...
    return (
        <div className="relative flex w-full cursor-grab items-center justify-between rounded-xl bg-white p-3 shadow-md active:cursor-grabbing">
                ...
                <div>
                    <div className="flex gap-2">
                        ...
                        {user === null ? null : (
                            <>
                                <div className="h-8 w-8 bg-gray-300 rounded-full flex justify-center items-center">
                                    <Link title="Show Url" href={route('urls.show', id)} className="flex justify-center items-center relative group h-full w-full rounded-full transition-all duration-75 border border-gray-200 bg-gray-50 hover:bg-yellow-100 hover:border-yellow-200">
                        ...
                        <div>
                            <Dropdown>
                                ...
                                <Dropdown.Content>
                                    <Dropdown.Link href={route('profile.edit')}>Clone</Dropdown.Link>
                                    <Dropdown.Link href={route('profile.edit')}>Copy</Dropdown.Link>
                                    <Dropdown.Link href={route('profile.edit')}>Code QR</Dropdown.Link>
                                    <Dropdown.Link href={route('profile.edit')}>Show</Dropdown.Link>
                                    <Dropdown.Link href={route('profile.edit')}>Edit</Dropdown.Link>
                                    <Dropdown.Link href={route('profile.edit')}>Delete</Dropdown.Link>                                    
                                </Dropdown.Content>
                            </Dropdown>
                        </div>
                    </div>
